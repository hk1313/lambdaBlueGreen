module "lambda" {
  ...
  function_name          = "test-lambda-bluegreen-${var.env}"
...
module "deploy" {
  ...
  create_app = true
  app_name   = "test-codedeploy-app-bluegreen-lambda-${var.env}"

  create_deployment_group = true
  deployment_group_name   = "test-codedeploy-deployment-group-bluegreen-lambda-${var.env}"
